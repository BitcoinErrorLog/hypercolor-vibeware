...
Persisted rows are allowlisted coarse events only, with `model_allowed=true`. Every payload field is a closed enum (`PAYLOAD_ENUMS` is exhaustive for `PAYLOAD_FIELDS`). Raw evidence expires after 14 days and is deleted. The SQL view `vibeware_evidence_projection` is the only agent and evaluate read path (`GET /v1/projection` and `POST /v1/experiments/:id/evaluate`). Raw `evidence` rows are never selected on those paths.
