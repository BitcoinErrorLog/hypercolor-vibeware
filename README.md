...
| `VIBEWARE_INGEST_TOKEN` | yes | Bearer token for `POST /v1/evidence` and `GET /v1/experiments/:id/assignment`. Must differ from the other two tokens |
...
| `VIBEWARE_INTERNAL_TOKEN` | yes | Bearer token for detect, qualify, generate, experiments (create, assign, kill, evaluate), and `POST /internal/gc`. Assignment also accepts the ingest token. Must differ from the other two tokens |
...
| `VIBEWARE_INGEST_ORIGINS` | no | Comma-separated exact origins (`scheme://host[:port]`) allowed to call `POST /v1/evidence` and `GET /v1/experiments/:id/assignment` from a browser. Default `https://hypercolor-web.vercel.app`. Boot throws on `*`, empty entries, `null`, paths, query strings, credentials, or non-http(s) values |
...
- `GET /v1/experiments/:id/assignment?cohort_key=` — ingest or internal bearer (timing-safe). Dashboard bearer and dashboard cookie are `401`. Response is `{bucket, experiment_id, killed}` only. Stable hash of `experiment_id + cohort_key` into 100 buckets; `candidate` if `unit < percent`, else `control`. After kill, every cohort is `control`. `cohort_key` is 64 hex and is not stored. OPTIONS and GET use the same closed ingest CORS allowlist as evidence (`Vary: Origin`; `Access-Control-Allow-Origin` only for an exact allowlisted Origin; no credentials). Create, kill, and evaluate stay internal-only and send no CORS.
